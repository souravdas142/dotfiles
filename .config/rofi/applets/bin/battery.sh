...
run_cmd() {
	...
	if [[ "$1" == '--opt1' ]]; then
		notify-send -h string:private-synchronous:rofibatsh -u low " Remaining : ${percentage}%"
	elif [[ "$1" == '--opt2' ]]; then
		notify-send -h string:private-synchronous:rofibatsh -u low "$ICON_CHRG Status : $status"
#	elif [[ "$1" == '--opt3' ]]; then
